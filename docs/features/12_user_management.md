...
　　管理番号の確認は同時登録で二重使用されないよう排他ロック付きで行っている（追加実装6・修正3）<br>
　　ユーザー名重複を確認する<br>
　　ユーザー名重複の確認はDBのユニーク制約でも保証しており、同時登録時はエラーになるようになっている（修正4・追加実装5）<br>
...
　利用停止中かどうかの判定はSpring SecurityのDisabledExceptionをログイン失敗時にAPI側で判定する方式になっている（修正6・修正7・修正8）<br><br>
